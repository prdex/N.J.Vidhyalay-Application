...
public class videolectureclassification extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.videoclassficationlayout);
        ...
        SharedPreferences settingsindex = getSharedPreferences("videoindex", Context.MODE_PRIVATE);
        String indexref = settingsindex.getString("videoindexsubject", String.valueOf(0));

        if(indexref.equals("B.A.")){
            myRef = database.getReference().child(linkref).child("linkdetails").child("BA");
        }
        else if(indexref.equals("S.P.C.C.")){
            myRef = database.getReference().child(linkref).child("linkdetails").child("SPCC");
        }else{
            myRef = database.getReference().child(linkref).child("linkdetails").child(indexref);
        }

       // DatabaseReference myRef = database.getReference().child(linkref).child("linkdetails").child(indexref);
        myRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for (DataSnapshot ds : dataSnapshot.getChildren()) {
                    ...
                    final TableRow trHead = new TableRow(videolectureclassification.this);
                    final TableRow linkhead = new TableRow(videolectureclassification.this);
                ...
                findViewById(R.id.loadingPanelvideoclassification).setVisibility(View.GONE);
            }
